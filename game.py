# To speed up training and simulating, we use the environment provided in the
# following source because it is many times faster than the implementation given
# with this homework: https://github.com/alanhyue/RL-2048-with-n-tuple-network
...
class Board:
    ...
    def is_legal(self, a):
        ...
        if a == LEFT:
            self.merge_to_left()
        if a == RIGHT:
            self.rotate().rotate().merge_to_left()
            self.rotate().rotate()
        if a == UP:
            self.rotate().rotate().rotate().merge_to_left()
            self.rotate()
        if a == DOWN:
            self.rotate().merge_to_left()
            self.rotate().rotate().rotate()
        if original == self.board:
            return False
        return True